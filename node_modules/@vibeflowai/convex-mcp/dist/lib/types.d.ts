import type { Annotations, BlobResourceContents, ContentBlock, GetPromptResult, PromptArgument, PromptMessage, ReadResourceResult, Role, TextResourceContents, ToolAnnotations } from "@modelcontextprotocol/sdk/types.js";
...
export type ConvexValidatorJson = {
    type: "string" | "number" | "float64" | "boolean" | "null" | "any" | "bytes";
} | {
    type: "id";
    tableName?: string;
} | {
...
export type ToolArgsBuilder = ToolArgsSchema & typeof import("zod")["z"] & {
    inferred: ToolArgsSchema;
    shape: ToolArgsShape;
};
export type ToolArgsInput = ToolArgsShape | ToolArgsSchema | ((builder: ToolArgsBuilder) => ToolArgsShape | ToolArgsSchema);
type ArgsSchemaFromInput<TArgs extends ToolArgsInput> = TArgs extends (...args: never[]) => infer R ? R extends ToolArgsSchema ? R : R extends ToolArgsShape ? z.ZodObject<R> : ToolArgsSchema : TArgs extends ToolArgsSchema ? TArgs : TArgs extends ToolArgsShape ? z.ZodObject<TArgs> : ToolArgsSchema;
...
export type PromptArgsOutput<TArgs extends PromptArgsInput | undefined> = TArgs extends PromptArgsInput ? z.output<ArgsSchemaFromInput<TArgs>> : Record<string, never>;
export type PromptResponse = GetPromptResult | PromptMessage | PromptMessage[];
export type PromptHandler<TArgs extends PromptArgsInput | undefined = undefined> = (args: PromptArgsOutput<TArgs>) => PromptResponse | Promise<PromptResponse>;
export interface PromptOptions<TArgs extends PromptArgsInput | undefined = undefined> {
    title?: string;
    description?: string;
    args?: TArgs;
}
...
export type ResourceParamsOutput<TParams extends ResourceParamsInput | undefined> = TParams extends ResourceParamsInput ? z.output<ArgsSchemaFromInput<TParams>> : Record<string, never>;
...
export interface ResourceTemplateOptions<TParams extends ResourceParamsInput | undefined = undefined> {
    kind: ConvexReadKind;
    uriTemplate: string;
    title?: string;
    description?: string;
...
export type ResourceContentInput = RawTextResourceContent | RawBlobResourceContent | RawJsonResourceContent;
export interface RawResourceResult {
    contents: ResourceContentInput[];
}
export type ResourceResponse = ReadResourceResult | RawResourceResult | ResourceContentInput | ResourceContentInput[] | string | number | boolean | null | Record<string, unknown> | unknown[];
...
export type McpHttpCtx = Pick<GenericActionCtx<any>, "runAction" | "runMutation" | "runQuery">;
