import { z } from "zod";
function asJsonSchemaType(value) {
    switch (typeof value) {
        case "string":
            return "string";
        case "number":
...
function recordValueSchema(validator) {
    return validator.value ?? validator.values ?? { type: "any" };
}
function objectFields(validator) {
    return Object.entries(validator.value);
}
export function convexValidatorToJsonSchema(validator) {
    switch (validator.type) {
        ...
        case "record":
            return {
                type: "object",
                additionalProperties: convexValidatorToJsonSchema(recordValueSchema(validator)),
            };
        case "object": {
            const properties = {};
            const required = [];
...
export function convexValidatorToZod(validator) {
    switch (validator.type) {
        case "string":
        case "id":
            return z.string();
...
export function convexArgsToZodShape(validator) {
...
export function convexArgsToZodObject(validator) {
    return z.object(convexArgsToZodShape(validator));
}
function asZodObject(schema) {
    return schema instanceof z.ZodObject ? schema : z.object(schema);
}
function createArgsBuilder(inferred) {
    return Object.assign(inferred, z, {
        inferred,
        shape: inferred.shape,
    });
}
function dropSchemaKeyword(schema) {
    const { $schema: _schema, ...rest } = schema;
    return rest;
}
export function normalizeToolArgs(validator, override) {
    const inferred = convexArgsToZodObject(validator);
    const next = typeof override === "function"
        ? override(createArgsBuilder(inferred))
        : (override ?? inferred);
    const objectSchema = asZodObject(next);
    return {
        inputShape: objectSchema.shape,
        inputSchemaJson: dropSchemaKeyword(z.toJSONSchema(objectSchema)),
    };
}
export function parseValidatorJson(serialized) {
