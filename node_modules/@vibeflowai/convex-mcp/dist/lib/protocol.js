...
import { toCallToolResult, toErrorToolResult, toReadResourceResult, } from "./result.js";
import { errorResponse, okResponse } from "./rpc.js";
import { invokeTool } from "./tools.js";
export async function handleMessage(ctx, tools, prompts, resources, resourceTemplates, serverInfo, protocolVersion, message) {
    ...
    switch (method) {
        case "initialize": {
            const requestedVersion = typeof rpc.params?.protocolVersion === "string"
        ...
        case "resources/read": {
            ...
            if (typeof uri !== "string") {
                return errorResponse(id, -32602, 'Invalid params: "uri" must be a string.', protocolVersion);
            }
            const resolved = findResourceByUri(resources, resourceTemplates, uri);
            if (!resolved) {
                return errorResponse(id, -32602, `Unknown resource "${uri}".`, protocolVersion);
            }
            if (resolved.kind === "resource") {
                try {
                    const value = await invokeResource(ctx, resolved.definition, {});
                    return okResponse(id, toReadResourceResult(value, {
                        uri,
                        mimeType: resolved.definition.mimeType,
                    }), protocolVersion);
                }
                catch (error) {
                    return errorResponse(id, -32603, error instanceof Error ? error.message : String(error), protocolVersion);
                }
            }
            ...
            if (!parsedArgs.success) {
                return errorResponse(id, -32602, `Invalid params for resource "${uri}": ${parsedArgs.error.message}`, protocolVersion);
            }
            try {
        ...
        case "prompts/get": {
            ...
            if (typeof promptName !== "string") {
                return errorResponse(id, -32602, 'Invalid params: "name" must be a string.', protocolVersion);
            }
            const prompt = prompts.get(promptName);
            if (!prompt) {
                return errorResponse(id, -32602, `Unknown prompt "${promptName}".`, protocolVersion);
            }
            const parsedArgs = await z
                .object(prompt.inputShape)
                .safeParseAsync(rpc.params?.arguments ?? {});
            if (!parsedArgs.success) {
                return errorResponse(id, -32602, `Invalid params for prompt "${promptName}": ${parsedArgs.error.message}`, protocolVersion);
            }
            try {
                return okResponse(id, toPromptResult(await prompt.handler(parsedArgs.data)), protocolVersion);
            }
            catch (error) {
                return errorResponse(id, -32603, error instanceof Error ? error.message : String(error), protocolVersion);
            }
        ...
        case "tools/call": {
            ...
            if (!tool) {
                return errorResponse(id, -32602, `Unknown tool "${toolName}".`, protocolVersion);
            }
            const parsedArgs = await z
                .object(tool.inputShape)
                .safeParseAsync(rpc.params?.arguments ?? {});
            if (!parsedArgs.success) {
                return errorResponse(id, -32602, `Invalid params for tool "${toolName}": ${parsedArgs.error.message}`, protocolVersion);
            }
            try {
