...
async function sendViaAgentmail(args) {
    ...
    const response = await axios.post(`https://api.agentmail.to/v0/inboxes/${args.inboxId}/messages/send`, {
        to: args.to,
...
server.registerTool("parseOutreachRequest", {
...
server.registerTool("generateVoiceAssets", {
    ...
    try {
        const aiRes = await axios.post("https://api.openai.com/v1/chat/completions", {
            ...
            messages: [
                {
                    role: "system",
                    content: "Create concise, professional voice outreach assets. Return strict JSON with keys personalizedPitch and audioScript.",
                },
                {
                    role: "user",
...
server.registerTool("matchRecruiters", {
    ...
    try {
        ...
        const scoreBreakdown = limited.map((r) => ({
            ...
            totalScore: (r.focus.toLowerCase().includes(args.targetRole.toLowerCase()) ? 1 : 0) +
...
server.registerTool("draftOutreachEmail", {
    ...
    try {
        ...
        const enrichedBody = args.personalizedPitch || args.audioScript
            ? `${baseTemplate.body}
...
server.registerTool("sendOutreachEmail", {
...
export async function runOutreachPipeline(args) {
    ...
    try {
        ...
        const voiceRes = await axios.post("https://api.openai.com/v1/chat/completions", {
            model: "gpt-4o-mini",
            messages: [
                {
                    role: "system",
...
server.registerTool("runOutreachPipeline", {
...
server.registerTool("exportOutreachPipelineFlow", {
